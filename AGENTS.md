...
### 重要なメンテナンス
- **コマンドの追加/変更**: 新しいコマンドを追加したり仕様を変更した場合は、必ず `skills/deba/SKILL.md` を更新し、`npm run deba -- maintenance setup-skill` を実行して反映させてください。
- **整合性チェック**: `skills/deba/SKILL.md` の内容と実装の整合性は `test/skill_md.test.ts` で検証されます。機能追加時はこのテストも更新してください。
...
- **yolo モードによる自律性の追求**: Deba は AI エージェントの自律性を最大限に引き出すため、LLM CLI を `yolo` モード（確認なしの実行）で動作させることを基本方針とします。セキュリティ上の制約によってエージェントの思考が妨げられることを防ぎ、自己責任の下で「自律的なエンジニア」としての振る舞いを優先します。
...
- **Git / GitHub 操作の制限**: ユーザーから明示的に指示されていない場合、Git のコミット、プッシュ、マージ、および GitHub のプルリクエスト作成やマージを行うことは厳禁です。
