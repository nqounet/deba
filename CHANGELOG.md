...
## [0.11.16] - 2026-03-13
### Fixed
- Fixed an issue where `setup-skill` did not install the required Semantic Knowledge Repository (SKR) by making `setupSkillCommand` automatically trigger `setupSKRCommand`.

## [0.11.15] - 2026-03-13
### Added
- Supported yolo mode for AI providers.
### Fixed
- Prevented prompt injection by sanitizing user input and task data before embedding into LLM prompts in review and worker services.
- Improved worktree removal robustness by adding a fallback to forcefully delete the directory if standard git commands leave it behind.
