...
Deba は、AI エージェントが自律的にタスクを遂行することを前提に設計されています。このため、既定の LLM CLI である `gemini` を使用する場合、**`--yolo` モード（You Only Look Once）での実行を基本方針としています。**

- **自律性の追求**: AI が生成したコマンドを安全確認の対話なしに直接実行することで、本来のパフォーマンスと開発速度を維持します。
- **リスクの許容と隔離**: この方針は一定のリスクを伴いますが、Deba は Git Worktree による隔離実行などの安全策を講じています。利用者はツールの自律性を尊重し、発生するリスクについて同意したものとみなします。
