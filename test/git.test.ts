...
`;
      vi.mocked(execFileSync).mockReturnValue(mockOutput as any);

      const url = getRemoteOriginUrl();
      expect(url).toBe('ssh://git@github.com/nqounet/deba.git');
      expect(execFileSync).toHaveBeenCalledWith('git', ['remote', '-v'], { encoding: 'utf8' });
    });
...
`;
    ...
    it('origin が見つからない場合にエラーを投げること', () => {
      vi.mocked(execFileSync).mockReturnValue(`upstream	ssh://... (fetch)\n` as any);
      expect(() => getRemoteOriginUrl()).toThrow('origin remote not found');
    });

    it('execFileSync が失敗した場合にエラーを投げること', () => {
  ...
  describe('getRepoStorageRoot', () => {
    it('URLから正しいストレージパスを生成し、ディレクトリが存在しない場合は作成すること', () => {
      vi.mocked(execFileSync).mockReturnValue(`origin	ssh://git@github.com:nqounet/deba.git (fetch)\n` as any);
    ...
    it('https URL の場合', () => {
      vi.mocked(execFileSync).mockReturnValue(`origin	https://github.com/nqounet/deba.git (fetch)\n` as any);
  ...
  describe('getMainRepoRoot', () => {
    it('メインリポジトリの場合、toplevel を返すこと', () => {
      vi.mocked(execFileSync).mockImplementation((cmd, args) => {
        if (args && args.includes('--show-toplevel')) return '/path/to/repo' as any;
        if (args && args.includes('--git-common-dir')) return '.git' as any;
        return '' as any;
      });
    ...
    it('Worktree の場合、親リポジトリのルートを特定できること', () => {
      vi.mocked(execFileSync).mockImplementation((cmd, args) => {
        if (args && args.includes('--show-toplevel')) return '/path/to/main/worktrees/task1' as any;
        if (args && args.includes('--git-common-dir')) return '/path/to/main/.git/worktrees/task1' as any;
        return '' as any;
      });
...
`;
  ...
  describe('createWorktree', () => {
    it('worktreeディレクトリを作成し、gitコマンドを呼び出すこと', () => {
      vi.mocked(execFileSync).mockImplementation((cmd, args) => {
        if (args && args.includes('remote') && args.includes('-v')) return 'origin	ssh://git@github.com/nqounet/deba.git (fetch)\n' as any;
        if (args && args.includes('--show-toplevel')) return '/path/to/main' as any;
        if (args && args.includes('--git-common-dir')) return '.git' as any;
        if (args && args.includes('list') && args.includes('--porcelain')) return 'worktree /path/to/main\n' as any;
        return '' as any;
      });
      ...
      expect(execFileSync).toHaveBeenCalledWith('git', expect.arrayContaining(['worktree', 'add', '-b', 'feature/task_20260308_120000_abcdef12']), expect.objectContaining({ stdio: 'inherit' }));
  ...
  describe('removeWorktree', () => {
    it('git worktree remove を呼び出すこと', () => {
      ...
      expect(execFileSync).toHaveBeenCalledWith('git', ['worktree', 'remove', '/path/to/wt', '--force'], expect.any(Object));
      expect(execFileSync).toHaveBeenCalledWith('git', ['branch', '-D', 'feature/task_20260308_120000_abcdef12'], expect.any(Object));
    });
  });
  ...
  describe('mergeWorktree', () => {
    it('worktree 側でコミットし、メイン側で merge --squash すること', () => {
      ...
      expect(execFileSync).toHaveBeenCalledWith('git', ['add', '.'], expect.objectContaining({ cwd: expect.any(String) }));
      expect(execFileSync).toHaveBeenCalledWith('git', ['merge', '--squash', 'feature/task_20260308_120000_abcdef12'], expect.any(Object));
    });
  });
  ...
  describe('cleanWorktrees', () => {
    it('deba-wt- を含む全ての worktree を削除すること', () => {
      vi.mocked(execFileSync).mockImplementation((cmd, args) => {
        if (args && args.includes('list') && args.includes('--porcelain')) {
      ...
      expect(execFileSync).toHaveBeenCalledWith('git', ['worktree', 'remove', '/path/to/deba-wt-task_20260308_120000_abcdef12', '--force'], expect.any(Object));
    });

    it('削除対象がない場合は何もしないこと', () => {
      vi.mocked(execFileSync).mockReturnValue('worktree /main/repo\n' as any);
