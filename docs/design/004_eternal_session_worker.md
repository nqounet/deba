...
> **⚠️ DEPRECATED (非推奨)**:
> 本ドキュメントで提案されている「永続セッション（1つのチャットセッションを長時間維持する）」というアプローチは、トークン消費の爆発（コスト悪化）およびコンテキスト汚染による品質低下のリスクが極めて高いため、**廃止（非推奨）**となりました。詳細は `006_stateless_worker_architecture_adr.md` を参照してください。現在の Deba は Stateless Multi-Agent アーキテクチャを採用しています。
