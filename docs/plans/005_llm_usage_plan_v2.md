...
| フェーズ | 対象セクション | 内容 |
|---------|------------|------|
| **α版（必須）** | §1-7, §8(マトリクス本体), §9.1, §10-11 | コアのPlanning/B/Cフロー、Ingestion、コスト上限、エラーハンドリング基本、テストなし対応 |
...
> **トークン消費量の精緻化プロセス**: 上記は概算レンジである。α版の最初の20タスクで入出力トークン数を実測し、以下を更新する: (1) Planning/B/Cの平均トークン消費量、(2) コスト概算の精緻化、(3) 月間上限値の適正化。
...
| 実装困難 | **3回** (Planning + B escalation + C) | 1回 (Execution) | テスト実行 |
