...
| 要件 | 基準 |
|------|------|
| 構造化出力の安定性 | YAML/JSONフォーマットの出力を9割以上の確率でパース可能 |
...
│  [タスク実行ごと]                                              │
│  ┌────────────────────────────────────────────────────────┐  │
│  │ Pre-Phase: スコープ判定（ルールベース + 曖昧時のみ軽量モデル） │  │
│  │ - 要望に複数の独立変更が含まれるか判定                   │  │
│  │ - 含まれる場合 → ユーザーに分割を提案                   │  │
│  └────────────────────────────────────────────────────────┘  │
│                        ↓                                     │
...
│  │                                                        │  │
│  │ [タイムアウト(72h応答なし)] → LLM呼び出しなし           │  │
│  │   保留エピソードとして記録。次回応答時にPhase C実行      │  │
│  └────────────────────────────────────────────────────────┘  │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```
...
Phase A出力に対し、以下のバリデーションを自動実行する。バリデーション失敗時はフォールバック（§5.4）を発動する。

| バリデーション項目 | チェック内容 | 失敗時の対応 |
|------------------|------------|-------------|
| YAMLパース | 出力がvalid YAMLであること | 修復プロンプト（1回）→ 失敗でフォールバック |
| スキーマ検証 | 必須フィールド（goal, steps, acceptance_criteria）の存在 | フォールバック |
...
### 5.5 出力形式の選択（YAML vs JSON）

| 形式 | 利点 | 欠点 | 採用判断 |
|------|------|------|---------|
| YAML | 可読性が高い、コメント記載可能 | LLMの構造化出力モード非対応が多い、インデントエラーが起きやすい | **α版で第一候補として検証** |
| JSON | LLMのJSON Modeで安定出力可能、パーサーが堅牢 | 可読性が低い、コメント不可 | **YAMLパース失敗率が10%を超える場合に切り替え** |
...
```
Phase A出力 → YAMLパーサー → 依存グラフ構築 → 並列度決定
                                                ↓
                                ┌───────────────────────┐
                                │ 並列バッチ1            │
                                │ step1 → CLI#1 (軽量)  │
...
| バッチ間トランザクション | バッチN全完了 → テスト実行 → 失敗時は**バッチNの変更のみ**git revertしてから修正ループ（バッチN-1以前の変更は保持） |
...
軽量モデルが生成したコードのテストが失敗した場合、即座にプレミアムモデルにエスカレーションする。軽量モデルでのリトライは行わない（判断力の低いモデルで繰り返しても手戻りが増えるだけであるため）。

| 項目 | 内容 |
|------|------|
| トリガー | 軽量モデルが生成したコードのテスト失敗（1回で即発動） |
| 入力 | テスト失敗のエラーメッセージ + 該当コード + Phase Aの元計画 |
...
ユーザーの反応を判定
  ...
  │    → 終了
  │
  └─ [タイムアウト（72時間応答なし、設定変更可）]
       → 保留エピソードとして記録（LLM不要）
       → Worktreeは保持（自動削除しない）
       → 次回ユーザーが反応した時点でPhase C再開
```
...
| 改善機会の損失 | 動くが最適ではないコードを承認している場合がある | Consolidation（§9）でエピソード横断のパターン分析により検出を試みる |
...
| **Phase B** | テスト失敗 | 即座にPremiumエスカレーション | — | ユーザーに手動対応を依頼（該当ステップをスキップ） |
...
| APIキー/トークン | 連続20文字以上の英数字列 | `[REDACTED]` |
| IPアドレス | IPv4/IPv6パターン | `[IP_ADDR]` |

ユーザーはプロジェクトごとに追加のマスキングルールを設定可能。マスキングによりエラー文脈が失われ修正精度が下がるリスクがあるため、マスキング適用の有無はエピソードに記録し、Consolidation時の分析に活用する。
...
| モデル | Premium |
| 入力 | ソースコード全体（推定トークン数が100K以下の場合）or ファイルツリー + 主要ファイル抜粋（100K超過時） |
...
実行中のPhase Bを停止
  ↓
Worktree上の全変更を git revert
  ↓
現在のタスクを「中断」エピソードとして記録
（Phase Cは実行しない。学び抽出の対象外）
  ↓
新しい要望を新規タスクとしてPre-Phaseから開始
```
